...
def verificar_imposto(imposto):
    """Verifica se o imposto é conhecido. Se não for, tenta sugerir o mais parecido.
    Retorna (conhecido: bool, sugestao: str|None)."""
    imp = imposto.upper()
    if imp in IMPOSTOS_VALIDOS or imp in ALIAS_IMPOSTOS:
        return True, None
    # Procura o imposto conhecido mais parecido
    parecidos = difflib.get_close_matches(imp, IMPOSTOS_VALIDOS, n=1, cutoff=0.6)
    sugestao = parecidos[0] if parecidos else None
...
def parsear_mensagem(mensagem):
    ...
    # ── Alerta suave: imposto não reconhecido (não bloqueia) ──
    conhecido, sugestao = verificar_imposto(descricao)
    alerta_imposto = None
    if not conhecido:
        if sugestao:
            alerta_imposto = f'⚠️ _"{descricao}" não está na lista de impostos conhecidos. Você quis dizer *{sugestao}*? Se foi engano, cancele e recadastre._'
        else:
            alerta_imposto = f'⚠️ _"{descricao}" não está na lista de impostos conhecidos. Confira se está correto._'
    ...
    if alerta_imposto:
        resultado["alerta_imposto"] = alerta_imposto
